...
import { Pesquisa, PesquisaCategoria, PesquisaStatus } from "@prisma/client";
...
interface EditarPesquisaModalProps {
  pesquisa: Pesquisa;
  onPesquisaEditada?: () => void;
}

export function EditarPesquisaModal({ pesquisa, onPesquisaEditada }: EditarPesquisaModalProps) {
  const [open, setOpen] = useState(false);

  const defaultFormValues = {
    ...pesquisa,
    descricao: pesquisa.descricao ?? "",
    tituloProjeto: pesquisa.tituloProjeto ?? "",
    objetivoGeral: pesquisa.objetivoGeral ?? "",
    objetivosEspecificos: pesquisa.objetivosEspecificos ?? "",
    justificativa: pesquisa.justificativa ?? "",
    publicoAlvo: pesquisa.publicoAlvo ?? "",
    metodologia: pesquisa.metodologia ?? "",
    produtosEsperados: pesquisa.produtosEsperados ?? "",
    proponente: pesquisa.proponente ?? "",
    cnpjProponente: pesquisa.cnpjProponente ?? "",
    municipio: pesquisa.municipio ?? "",
    areaAbrangencia: pesquisa.areaAbrangencia ?? "",
    processoSei: pesquisa.processoSei ?? "",
    valorTotal: Number(pesquisa.valorTotal ?? 0),
    fonteRecurso: pesquisa.fonteRecurso ?? "",
    elementoDespesa: pesquisa.elementoDespesa ?? "",
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      const response = await fetch(`/api/admin/pesquisas/${pesquisa.id}`, {
        method: "PATCH",
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "Falha ao editar pesquisa.");
      }
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Ex: Pesquisa de Satisfação - ExpoAgro 2025" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                      ...
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent className="bg-white">
                            {CATEGORIAS_PESQUISA.map((categoria) => <SelectItem key={categoria.value} value={categoria.value}>{categoria.label}</SelectItem>)}
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                      ...
                      <FormItem>
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent className="bg-white">
                            ...
                            <SelectItem value={PesquisaStatus.CANCELADO}>Cancelado</SelectItem>
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea placeholder="Breve resumo sobre o objetivo da pesquisa..." {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
