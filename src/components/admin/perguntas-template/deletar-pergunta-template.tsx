...
export function DeletarPerguntaTemplateBotao({ perguntaId, onPerguntaDeleted }: DeletarPerguntaBotaoProps) {
  const [isDeleting, setIsDeleting] = useState(false);

  const handleDelete = async () => {
    setIsDeleting(true);
    try {
      const response = await fetch(`/api/admin/perguntas-template/${perguntaId}`, {
        method: 'DELETE',
      });
